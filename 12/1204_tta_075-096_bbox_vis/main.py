...
if __name__ == "__main__":
    ...
    img_dict = readfiles(input_dir, '.jpeg')

    for filename, json_path in tqdm(json_dict.items()):
        img_path = img_dict[filename]
        output_img_path = makeOutputPath(img_path, input_dir, output_dir, 'jpeg')
        img = read_img(img_path)
        data = readJson(json_path)

        char_info = data['label']['character'].get('char_info')
        if char_info:
            for char in char_info:
                x1 = round(char['bbox']['x'])
                y1 = round(char['bbox']['y'])
                x2 = x1 + round(char['bbox']['w'])
                y2 = y1 + round(char['bbox']['h'])

                cv2.rectangle(img, (x1,y1), (x2, y2), color=(0,0,255), thickness=3)
        
        save_img(output_img_path, img, 'jpeg')