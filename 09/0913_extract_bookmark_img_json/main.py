...
with open(bookmark_dir, 'r', encoding='utf-16') as f:
...
if file_type == 'jpg':
    for filename in tqdm(file_list, desc='파일복사중'):
        ...
        if img_path:
            copy_file(img_path, otuput_dir)
elif file_type == 'json':
    for filename in tqdm(file_list, desc='파일복사중'):
        json_path = json_dict.get(filename, otuput_dir)
        if json_path:
            copy_file(json_path)
