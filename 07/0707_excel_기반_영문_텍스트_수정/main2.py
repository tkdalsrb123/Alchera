    ...
    for i in property_group:
        ...
        if word in lower_sentence:    # 단어가 해당 문장에 있을 경우
            if len(word.split(' ')) == 1:   # word가 한 단어일 때
                if word in lower_sentence.split(' '):
                ...
                elif f"{word}'s" in lower_sentence.split(' ') or f"{word}," in lower_sentence.split(' ') or f"{word}." in lower_sentence.split(' '):
                    if sample == df['문장']:    # 매칭된 단어가 없어 변경이 되지 않았을 경우
                        if lower_sentence.count(word) == 1:    # 문장에 단어가 한개일 경우
...
raw_df.to_excel(f'{save_dir}/new2.xlsx', index=False)
